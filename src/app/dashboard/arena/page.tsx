...
import { Activity, Mic, ChevronRight, Play, Lock } from "lucide-react";
...
export default function ArenaPage() {
  ...
  const handleAcceptChallenge = (drillTypeId: string) => {
    // For now, we just route to the analyze page for video, or a placeholder for audio
    if (activeMode === "video") {
        router.push("/dashboard/analyze");
    } else {
        // Audio flow not implemented yet in this dashboard version
    ...
    <div className='p-8 pt-12 max-w-[1200px] mx-auto animate-fade-in-up'>
      <div className='mb-12'>
        ...
        <p className='text-lg text-gray-400 max-w-2xl'>
          Select a protocol to begin. Your performance will be verified, analyzed, and minted as an IP asset.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {activeDrills.map((drill) => (
          <div 
            ...
            onClick={() => handleAcceptChallenge(drill.drill_type_id)}
          ...
          >
            ...
            <div className="p-8 flex items-center gap-8 relative z-10">
                ...
                <div className={cn(
                    ...
                    activeMode === "video" ? "bg-blue-500/20 text-blue-400" : "bg-purple-500/20 text-purple-400"
                )}>
                    {activeMode === "video" ? <Activity className="w-8 h-8" /> : <Mic className="w-8 h-8" />}
                </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-zinc-900/20 p-8 flex items-center gap-8 opacity-50 cursor-not-allowed">
            ...
            <div>
                <h3 className="text-xl font-bold text-gray-500 mb-1">Locked Protocols</h3>
                <p className="text-gray-600 text-sm">
                    Advanced {activeMode === "video" ? "drills" : "capsules"} unlocking in Season 2.
                </p>
            </div>
        </div>
      </div>
