      ...
      <Card className='w-full max-w-2xl max-h-[85vh] bg-[#0a0a0a] border border-white/10 shadow-2xl relative z-10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-xs text-gray-400'>
                Review and register on Story Protocol
              </p>
            </div>
          </div>
          <button
        ...
        <div className='flex-1 p-6 overflow-y-auto scroll-smooth'>
          <div className='relative'>
            ...
            <button
              onClick={handleCopy}
              className='absolute top-3 right-3 p-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg transition-colors group cursor-pointer'
              title='Copy JSON'
            >
              {copied ? (
            ...
            <div className='p-4 rounded-xl bg-zinc-900/50 border border-white/5'>
              ...
              <p className='text-sm font-medium text-green-400'>
                {metadata.verification?.human_confidence_score
                  ? `${(
                      metadata.verification.human_confidence_score * 100
                    ).toFixed(0)}%`
                  : "N/A"}
              </p>
            </div>
